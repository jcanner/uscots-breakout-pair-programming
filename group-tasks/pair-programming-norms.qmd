...
::: {.callout-warning}
Note that these role and norm descriptions represent the pair programming protocols used in our 2024-2025 pilot. New protocols may be available! Please reach out to Judith or Allison for an update!
:::
...
In class, we provide a pair programming card with the following information. Here is a word version you can use in your own classroom.

[Pair Programming Card](pair-programming-norms.docx)
...
-   Talks with Developer about ideas related to ways they could solve
    the problem.
-   **Does not** ask the Developer how they would solve the problem.
-   Manages resources (e.g., cheatsheets, textbook) for aiding in
    solution strategy.
-   **Does not** ask the developer what functions / tools they should
    use.
...
-   Reads prompt and ensures Coder understands what the problem is
    asking.
...
-   Checks for accuracy by asking the solution to be restated for
    clarity.
...
-   Once code runs, evaluates the output relative to the prompt to
    ensure the output addresses the question.
...
<!-- Resource Manager --> What other sources of information do we need?\
...
solutions and justifications. **Everyone** must take turns writing the
final product (as described above) and **everyone** must be able to
explain every line of code in your final document.
